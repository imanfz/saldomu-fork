...
public class CameraClass {
    ...
    public final static int CAPTURE_IMAGE_ACTIVITY_REQUEST_CODE = 120;

    public CameraClass(Activity _context){
        this.setmActivity(_context);
    }
    ...
    public void RestoreInstanceState(@NonNull Bundle savedInstanceState)
    {
        if (savedInstanceState.containsKey(DATE_CAMERA_INTENT_STARTED_STATE)) {
            dateCameraIntentStarted = DateTimeFormat.convertStringtoCustomDateTime(
...
//              String sdkVersion = android.os.Build.VERSION.RELEASE.toLowerCase(Locale.ENGLISH);
                ...
                dateCameraIntentStarted = DateTimeFormat.getCurrDate();
...
//                    String filename = ImageConfig.getTempFileName();
                    ...
                    preDefinedCameraUri = getmActivity().getContentResolver().insert(
                            MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
                            values);
                    intent.putExtra(MediaStore.EXTRA_OUTPUT, preDefinedCameraUri);
                }
                getmActivity().startActivityForResult(intent, CAPTURE_IMAGE_ACTIVITY_REQUEST_CODE);
     ...
     *
     */
    public Uri onCameraIntentResult(int resultCode, Intent intent)
    {
        if (resultCode == Activity.RESULT_OK) {
            ...
            try {
                ...
                myCursor = getmActivity().getContentResolver().query(
                        MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
                        largeFileProjection,
                        null, null,
                        largeFileSort);
    ...
    {
        Toast.makeText(getmActivity().getApplicationContext(), getmActivity().getString(R.string.toast_error_could_not_take_photo), Toast.LENGTH_LONG).show();
    }
    ...
    {
        Toast.makeText(getmActivity().getApplicationContext(), getmActivity().getString(R.string.toast_error_sd_card_not_mounted), Toast.LENGTH_LONG).show();
    }
    ...
    private Uri getFileUriFromContentUri(Uri cameraPicUri)
    {
        try {
            if (cameraPicUri != null
                ...
                Cursor cursor = getmActivity().getContentResolver().query(cameraPicUri, proj, null, null, null);
                if (cursor != null) {
                    cursor.moveToFirst();
