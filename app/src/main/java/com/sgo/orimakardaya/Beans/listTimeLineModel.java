...
@Table(name = "PostTimeline")
public class listTimeLineModel extends Model {
    ...
    public listTimeLineModel() {
        super();
    }

    public listTimeLineModel(int _timeline_id, String _post, String _amount,
        ...
        super();
        this.setTimeline_id(_timeline_id);
        this.setPost(_post);
    ...
    public static List<listTimeLineModel> getAll() {
        // This is how you execute a query
        return new Select()
                .all()
                .from(listTimeLineModel.class).orderBy("timeline_id DESC")
                .execute();
    }

    public static void deleteAll() {
        // This is how you execute a query
        new Delete().from(listTimeLineModel.class).execute();
        SQLiteUtils.execSql("DELETE FROM SQLITE_SEQUENCE WHERE name='PostTimeline';");
    }

    public static void updateNumlikes(String _numlikes, int _id){
        new Update(listTimeLineModel.class).set("numlikes = ?", _numlikes).where("timeline_id = ?", _id).execute();
    }

    public static void updateIsLike(String _islike, int _id){
        new Update(listTimeLineModel.class).set("isLike = ?", _islike).where("timeline_id = ?", _id).execute();
    }

    public static void updateLikes(String _likes, int _id){
        new Update(listTimeLineModel.class).set("likes = ?", _likes).where("timeline_id = ?", _id).execute();
    }

    public static void updateNumcomments(String _numcomments, int _id){
        new Update(listTimeLineModel.class).set("numcomments = ?", _numcomments).where("timeline_id = ?", _id).execute();
    }

    public static void updateComments(String _comments, int _id){
        new Update(listTimeLineModel.class).set("comments = ?", _comments).where("timeline_id = ?", _id).execute();
    }

    public static void updateCommentId1(String _comment_id_1, int _id){
        new Update(listTimeLineModel.class).set("comment_id_1 = ?", _comment_id_1).where("timeline_id = ?", _id).execute();
    }

    public static void updateCommentId2(String _comment_id_2, int _id){
        new Update(listTimeLineModel.class).set("comment_id_2 = ?", _comment_id_2).where("timeline_id = ?", _id).execute();
    }

    public static void updateFromname1(String _from_name_1, int _id){
        new Update(listTimeLineModel.class).set("from_name_1 = ?", _from_name_1).where("timeline_id = ?", _id).execute();
    }

    public static void updateFromname2(String _from_name_2, int _id){
        new Update(listTimeLineModel.class).set("from_name_2 = ?", _from_name_2).where("timeline_id = ?", _id).execute();
    }

    public static void updateFromprofilepicture1(String _from_profile_picture_1, int _id){
        new Update(listTimeLineModel.class).set("from_profile_picture_1 = ?", _from_profile_picture_1).where("timeline_id = ?", _id).execute();
    }

    public static void updateFromprofilepicture2(String _from_profile_picture_2, int _id){
        new Update(listTimeLineModel.class).set("from_profile_picture_2 = ?", _from_profile_picture_2).where("timeline_id = ?", _id).execute();
    }

    public static void updateReply1(String _reply_1, int _id){
        new Update(listTimeLineModel.class).set("reply_1 = ?", _reply_1).where("timeline_id = ?", _id).execute();
    }

    public static void updateReply2(String _reply_2, int _id){
        new Update(listTimeLineModel.class).set("reply_2 = ?", _reply_2).where("timeline_id = ?", _id).execute();
    }
